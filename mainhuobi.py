...
with open('%s%.13s.csv' % (LogCsvFile, time.time()), 'w', newline='') as f:  # newline=" "是为了避免写入之后有空行
    ff = csv.writer(f)
    ff.writerow(headers)
    f.close()
...
while 1:
    ...
    logger.info(u' 获取K线数据：%s ' % str(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())))
    ...
    Klines.clear()
    MA = talib.MA(data_df['open'], timeperiod=int(MTimePeriod), matype=0)  # 获取MA线值
    EEMA = talib.EMA(data_df['open'], timeperiod=int(TTimePeriod))  # 获取EMA线值
    ...
    if not BTC_Trade:
        ...
        if MA_EMA:
            ...
            if Order['status'] == 'error':
                ...
                BTC_Trade = True
                trade_time = Order['ts']
                trade_number = Order['data']['order_id']
        ...
        if Already_Judged:
            ...
            if time5 - trade_time >= trading_interval:
                Order = dm.get_contract_order_info(symbol=symbol, order_id=trade_number)
                ...
                if Order['status'] == 'error':
                    ...
                    with open(LogCsvFile, 'a', newline='') as f:  # newline=" "是为了避免写入之后有空行
                        ff = csv.writer(f)
                        ff.writerow(head)
                        f.close()
                    ...
                    if data_btc['status'] != "6":
                        ...
                        if len(Order1['data']['errors']) > 0:
                            ...
                            continue
                    elif data_btc['status'] == "4" or data_btc['status'] == "5" or data_btc['status'] == "6":
                        BTC_Trade = False  # 判断交易是否成功
                        Already_Judged = False  # 判断是否交易完成和撤销失败订单
                        Braiding = False
                        trade_time = None
            ...
            if Already_SELL_Judged:
                ...
                if time5 - trade_time >= trading_interval:
                    ...
                    logger.info(
                        "如果时间差大于定额时间：%s%s%s" % (times, trading_interval, time5 - trade_time >= trading_interval))
                    trading_interval = 1
                    BTC_Trade = False  # 判断交易是否成功
                    Already_Judged = False  # 判断是否交易完成和撤销失败订单
                    Braiding = False
            ...
            if MA_EMA:
                ...
                Order_sell.clear()
    time6 = time.clock()  # 获取时间0x01
    if time6 - time1 <= 60:
        time.sleep(60 - (time6 - time1))
